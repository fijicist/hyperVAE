...
z [batch, 32] + jet_type [batch] ──→ Latent Expansion
                    ...
                    Masks: particle_mask      L-GATr → Particles [B, 60, 4]
                           hyperedge_mask     MLP → Edges [B, M, 5]
                                              MLP → Hyperedges [B, K, 2]
...
```python
MLP(
    in_features=32,       # From latent
    hidden=96,
    layers=5,
    activation=ReLU,
    dropout=0.3,          # Higher dropout (0.3) to combat overfitting
    residual=True
)
→ Output: [B, max_edges, 5]
```
...
→ Output: [B, max_hyperedges, 2]
```

#### 6. Jet Feature Decoder
...
L_total = w_particle × L_particle 
        + w_aux × (w_edge × L_edge + w_hyperedge × L_hyperedge)
        + w_jet × L_jet
        + β(epoch) × w_kl × L_KL
```
...
- `w_edge = 2500.0`, `w_hyperedge = 1500.0`: Auxiliary weights
- `w_aux = 0.1`: Global auxiliary multiplier
- `w_jet = 6000.0`: Jet feature weight
...
**Why Mean Matching?**
- Edges are derived from particles (not independent)
- Only need correct **distribution** of edge features
- Individual edge-to-edge matching is computationally expensive

**Implementation**:
```python
def _edge_reconstruction_loss(batch, pred_features):
    true_features = batch.edge_attr  # [M_true, 5]
    pred_features = pred_features.reshape(-1, 5)  # [M_pred, 5]
    
    true_mean = true_features.mean(dim=0)  # [5]
    pred_mean = pred_features.mean(dim=0)  # [5]
...
**Loss Weight**:
```
L_edge_total = 0.1 × 2500.0 × L_edge = 250.0 × L_edge
```

Much smaller than particle loss (12000.0) → **auxiliary guidance only**.

### 3. Hyperedge Feature Loss (Auxiliary)

Similar to edge loss, matches **mean hyperedge features**:

```
L_hyperedge = MSE(mean(pred_hyperedges), mean(true_hyperedges))
```

**Loss Weight**:
```
L_hyperedge_total = 0.1 × 1500.0 × L_hyperedge = 150.0 × L_hyperedge
```
...
```
L_jet_total = 6000.0 × L_jet
```

Comparable to particle loss → strong constraint on jet properties.
...
training:
  ...
  loss_weights:
    ...
    edge_features: 2500.0
    hyperedge_features: 1500.0
    jet_features: 6000.0
    kl_divergence: 0.3
...
### 3. Why Auxiliary Losses?

**Problem**: Edges/hyperedges are derived from particles (not independent).

**Approach**:
1. **Primary**: Particle Chamfer loss (12000.0 weight)
2. **Auxiliary**: Edge/hyperedge distribution matching (250.0 / 150.0 weights)

**Rationale**:
- Particles determine everything else (4-momentum conservation)
- Auxiliary losses provide **weak guidance** for graph structure
- Prevents model from ignoring graph features entirely

**Result**: Better particle arrangements (physically meaningful graphs).
...
4. **Auxiliary Losses**: Graph structure guidance
5. **Annealing**: KL + temperature schedules for stable training

**Key Innovation**: Squared distance Chamfer loss prevents gradient vanishing → faster, more stable training.
